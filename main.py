...
def send_data(cmd_type):
    ...
    # print(frame)
    recived_frame = com_serial(frame)
    # print(recived_frame)
...
def publish_data(client, ret, cmd):
    if (ret != -1):
        client.connect(AWS_HOST, AWS_PORT, keepalive=60)
        if(cmd == TEMPERATURE):
            print('Temperatura: {} °C '.format(ret))
            # data = {
            #     "tipo": "temperatura",
            #     "valor": value
            # }
